(function(){
  ...
  function renderLevelPanel(){
    ...
    panel.innerHTML='<div class="vip-progress-card"><div><b>'+esc(current.name)+'</b><span>'+(next?remain.toLocaleString()+' EXP until '+esc(next.name):'Maximum VIP level')+' <i class="fa-regular fa-circle-question"></i></span></div><button type="button" disabled><i class="fa-solid fa-lock"></i> Upgrade</button><div class="vip-progress-line"><span style="width:'+Number(state.progressPercent||0)+'%"></span></div></div><div class="vip-level-list">'+levels.map(function(x,i){return '<article class="vip-reward-row '+(i===state.currentLevelIndex?'active':'')+'"><div class="vip-timeline-dot"></div><h3>'+esc(x.name)+'</h3><button disabled><i class="fa-solid fa-lock"></i> Upgrade</button><div class="vip-reward-box"><span class="vip-coin-stack">VIP</span><strong>'+money(x.oneTimeBonus)+'</strong></div></article>'}).join('')+'</div>';
  }
