(function(){
  ...
  if (window.__NAGA_REFERRAL_SHARE_QR_V113__) return;
  window.__NAGA_REFERRAL_SHARE_QR_V113__ = true;
  ...
  function nativeShareNow(){
    ...
    // Do not block on navigator.canShare({files}). On several iOS Safari/Chrome
    // versions it returns false even though navigator.share({files}) succeeds.
  ...
  document.addEventListener('click', function(e){
    ...
    if(!qrReady || !qrFileCache){
      showCopyResult(false, qrPreparing ? 'QR code is preparing. Please wait a moment, then tap Share again.' : 'QR image is still preparing. Please wait a moment and tap Share again.');
